...
const en: LocaleType = {
  ...
  Compare: {
    ...
    SelectModels: "Select Platforms",
    SelectModelsHint: (min: number, max: number, current: number) =>
      `Select ${min} to ${max} platforms, currently ${current} selected`,
    History: "Compare History",
    HistoryTitle: "Compare History",
    HistoryCount: (count: number) => `${count} records`,
    HistoryEmpty: "No compare history yet",
    ...
    TooFewModels: (min: number) => `Select at least ${min} platforms`,
    TooManyModels: (max: number) => `Select at most ${max} platforms`,
    MissingApiKey: (names: string) => `Please configure API key for ${names}`,
    TextOnlyHint: "Type a question to compare across selected platforms…",
    Collapse: "Collapse",
    Expand: "Edit",
    Done: "Done",
    ChangePlatforms: "Change",
    ComparingSummary: (count: number) => `Comparing ${count} platforms`,
